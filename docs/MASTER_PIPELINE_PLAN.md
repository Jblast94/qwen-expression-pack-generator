...
- **Generators are swappable.** Imagine first, Qwen/RunPod fallback, HF Spaces as CPU workers.
...
[Image Router]
   ...
   └── Qwen-Image-Edit-NSFW / RunPod Flash (NSFW, filtered, heavy) → fallback
        ↓
[Asset Store] — MinIO (objects) + Postgres/pgvector (metadata + embeddings)
        ↓
[Gallery / Export] — Immich or PhotoPrism (live) + nightly ZIP archives
        ↓
...
**Known issues to fix:**
- RunPod worker returns the *same* image (no actual edit). Needs prompt/payload audit.
...
**Immediate upgrades:**
1. Add `gallery` output mode (write to MinIO + Postgres row, skip ST packaging by default).
2. Add `imagine` backend toggle (call Grok Imagine API when safe; fall back to Qwen).
3. Fix RunPod worker: verify endpoint, payload schema, and that it actually triggers inference.
...
- **Routing rule:** If prompt is safe → Imagine. If moderated/filtered/NSFW → queue to RunPod/Qwen fallback.
...
**Target:**
- **One Postgres + pgvector** for: products, posts, assets, performance, agent memory, embeddings.
...
- **RunPod Flash** for GPU-heavy fallback generation.
...
| **Sprite Engineer** | Fixes qwen-expression-pack-generator | Gallery mode + Imagine toggle + RunPod fix |
...
| **Storage Consolidator** | One Postgres + MinIO + Valkey | Schema + migration plan |
| **Orchestrator** | Dagger + n8n + HF Spaces glue | First end-to-end Dagger pipeline |
| **Monetization Scout** | Scraping + scoring + affiliate | One category MVP |
| **Gallery Curator** | Immich/PhotoPrism + ZIP export | Live gallery + nightly archive |
| **QA / Tester** | Runs the loop, logs failures | Daily smoke test of full pipeline |
...
- Gallery output, Imagine toggle, RunPod fix.
- Test: one reference image → gallery + ZIP, no ST required.

**Phase 2 — Storage consolidation (2–3 days)**
- Single Postgres schema, MinIO buckets, Valkey only for cache.
